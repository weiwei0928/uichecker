...
class LeftTouchProcessor : TouchProcessor() {
        ...
        when (event.action) {
            ...
            2 -> if (this.hasContentTouched) {
                ...
                if (deltaX2 < 0.0f) {
                    ...
                    isSliding = true
                } else if (contentView.right + deltaX2 <= originalRect.right) {
                    ViewCompat.offsetLeftAndRight(contentView, deltaX2.toInt())
                    isSliding = true
                }
                if (isSliding) {
                    ...
                    val scrollDistance = max(
                        (originalRect.right - (contentView.right + deltaX2)).toDouble(),
                        0.0
                    ).toFloat()
                    val alpha = ((1.0f - min(
                        1.0,
    ...
    private fun smoothDismiss(dialog: YOYODialog, offsetX: Int, slope: Float) {
        ...
        valueAnimator.addUpdateListener(object : AnimatorUpdateListener {
            ...
            override fun onAnimationUpdate(animation: ValueAnimator) {
                ...
                val distance = originalRect.width()
                val scrollDistance =
                    max((originalRect.right - (contentView.right + deltaX)).toDouble(), 0.0)
                        .toFloat()
                val alpha =
                    ((1.0f - min(
                        1.0,
