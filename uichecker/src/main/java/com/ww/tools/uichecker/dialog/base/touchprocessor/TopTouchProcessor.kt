...
class TopTouchProcessor : TouchProcessor() {
        ...
        when (event.action) {
            ...
            2 -> if (this.hasContentTouched) {
                ...
                if (deltaY2 < 0.0f) {
                    ...
                    isSliding = true
                } else if (contentView.bottom + deltaY2 <= originalRect.bottom) {
                    ViewCompat.offsetTopAndBottom(contentView, deltaY2.toInt())
                    isSliding = true
                }
                if (isSliding) {
                    ...
                    val scrollDistance = max(
                        (originalRect.bottom - (contentView.bottom + deltaY2)).toDouble(),
                        0.0
                    ).toFloat()
                    val alpha = ((1.0f - min(
                        1.0,
    ...
    private fun smoothDismiss(dialog: YOYODialog, offsetY: Int, slope: Float) {
        ...
        valueAnimator.addUpdateListener(object : AnimatorUpdateListener {
            ...
            override fun onAnimationUpdate(animation: ValueAnimator) {
                ...
                val distance = originalRect.height()
                val scrollDistance =
                    max((originalRect.bottom - (contentView.bottom + deltaY)).toDouble(), 0.0)
                        .toFloat()
                val alpha =
                    ((1.0f - min(
                        1.0,
    ...
    private fun smoothBack(dialog: YOYODialog, offsetY: Int, slope: Float) {
        val contentView = dialog.contentView
        val valueAnimator = ValueAnimator.ofFloat(0.0f, offsetY.toFloat()).setDuration(400L)
        valueAnimator.addUpdateListener(object : AnimatorUpdateListener {
            private var lastX = 0
            private var lastY = 0
